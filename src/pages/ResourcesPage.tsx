      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="hidden lg:flex flex-1 items-center justify-center space-x-8 font-corporate">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} className="text-white hover:text-white/80 font-medium text-sm transition-all">{link.name}</a>
            ))}
          </div>
        ...
        {isOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="px-4 pt-2 pb-6 space-y-1 font-corporate">
              {navLinks.map((link) => (
                <a key={link.name} href={link.href} className="block px-3 py-4 text-sm font-semibold text-white hover:bg-white/5 rounded-xl" onClick={() => setIsOpen(false)}>{link.name}</a>
              ))}
              <div className="pt-4">
                <a href="/#contact" className="block w-full text-center bg-white text-vento-navy px-6 py-4 text-xs font-bold uppercase tracking-wider rounded-full" onClick={() => setIsOpen(false)}>
                  Request A Quote
                </a>
              </div>
            </div>
...
const Footer = () => (
  <footer className="bg-vento-navy border-t border-white/10 py-10">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
      ...
      <Link to="/" className="text-white/30 hover:text-white/60 text-xs font-corporate transition-colors">← Back to Home</Link>
    </div>
  </footer>
);
